...
function DodajProdukt($conn) {
    if (isset($_POST['dodaj_produkt'])) {
       ...
       $stmt = $conn->prepare("INSERT INTO produkty (tytul, opis, cena_netto, podatek_vat, ilosc_sztuk, status_dostepnosci, kategoria_id, gabaryt, zdjecie, size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssdidsisss", $tytul, $opis, $cena_netto, $podatek_vat, $ilosc_sztuk, $status_dostepnosci, $kategoria_id, $gabaryt, $zdjecie, $size);
...
function EdytujProdukt($conn, $id) {
    ...
    if (isset($_POST['edytuj_produkt'])) {
       ...
       $stmt = $conn->prepare("UPDATE produkty SET tytul = ?, opis = ?, cena_netto = ?, podatek_vat = ?, ilosc_sztuk = ?, status_dostepnosci = ?, kategoria_id = ?, gabaryt = ?, zdjecie = ?, size=? WHERE id = ? LIMIT 1");
        $stmt->bind_param("ssdidsisssi", $tytul, $opis, $cena_netto, $podatek_vat, $ilosc_sztuk, $status_dostepnosci, $kategoria_id, $gabaryt, $zdjecie, $size, $id);
        ...
        <input type='number' name='ilosc_sztuk' id='ilosc_sztuk' value='" . $produkt['ilosc_sztuk'] . "'><br><br>
         ...
         <label for='kategoria_id'>Kategoria:</label><br>
          ...
          <label for='zdjecie'>Zdjęcie (Link):</label><br>
           ...
           <label for='size'>Size:</label><br>
            <input type='text' name='size' id='size' value='" . htmlspecialchars($produkt['size']) . "'><br><br>
